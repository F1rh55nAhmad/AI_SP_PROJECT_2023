...
class Ui_AISearchingTechniquesMainWindow(object):
    ...
    graphbbfs = Graph_astar(directed=False)
    graphabbfsD = Graph_astar(directed=True)
        ...
        with open('test.txt', 'w') as f:
            ...
            if graphType == "Undirectd Graph":
                if searchType == "BFS":
                    ...
                    if (traced_path1):
                        print('Path:', end=' ')
                        Graph_bfs.print_path(self, traced_path1, goal)
                        print()
                    Goal_list.clear()
                    ...
                    if (traced_path2):
                        print('Path:', end=' ')
                        Graph_dfs.print_path(self, traced_path2, goal)
                        print()
                    Goal_list.clear()
                    ...
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, cost3, goal = graphastar.astar_search(start, goals)
                    if (traced_path5):
                        print('Path:', end=' ')
                        graphastar.print_path(traced_path5, goal)
                        print('\nCost:', cost3)
                    Goal_list.clear()
                elif searchType == "Best First Search":
                    graphbbfss = Graph_best_first(directed=False)
                    for i in range(0, self.counter):
                        graphbbfss.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, cost3, goal = graphbbfss.best_first_search(start, goals)
                    if traced_path5:
                        print('Path:', end=' ')
                        graphbbfss.print_path(traced_path5, goal)
                        print('\nCost: ', cost3)
                    Goal_list.clear()
                elif searchType == "Uniform Cost Search":
                    graphucs = Graph_ucs(directed=False)
                    for i in range(0, self.counter):
                        graphucs.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, cost3, goal = graphucs.uniform_cost_search(start, goals)
                    if traced_path5:
                        print('Path:', end=' ')
                        graphucs.print_path(traced_path5, goal,cost3)
                        print('\nCost: ', cost3)
                    Goal_list.clear()
                elif searchType == "Depth limited Search":
                    graphdls = Graph_dls(directed=False)
                    for i in range(0, self.counter):
                        graphdls.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    limit =10
                    traced_path5, goal,limit = graphdls.depth_limited_search(start, goals,limit)
                    if traced_path5:
                        print('Path:', end=' ')
                        graphdls.print_path(traced_path5, goal)
                    Goal_list.clear()
                elif searchType == "IterativeDeepning":
                    graphids = Graph_ids(directed=False)
                    for i in range(0, self.counter):
                        graphids.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    max_depth =10
                    traced_path5, goal = graphids.depth_limited_search(start, goals,max_depth)
                    if traced_path5:
                        print('Path:', end=' ')
                        graphids.print_path(traced_path5, goal)
                    Goal_list.clear()
                elif searchType == "Bidirectional":
                    graphbd = Graph_bd(directed=False)
                    for i in range(0, self.counter):
                        graphbd.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, goal = graphbd.bidirectional_search(start, goals)
                    if traced_path5:
                        graphbd.print_path(traced_path5, goal)
                    Goal_list.clear()
                elif searchType == "SimulatedAnnealing":
                    graphsa = SimulatedAnealing(directed=False)
                    for i in range(0, self.counter):
                        graphsa.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    result = graphsa.simulated_annealing_search(start, goals)
                    if isinstance(result, tuple) and len(result) >= 2:
                        traced_path5 = result[0]
                        goal = result[1]
                        print('Path:', end=' ')
                        graphsa.print_path(traced_path5)
                    else:
                ...
                elif searchType == "AlphaBetaPrunning":
                    graphabp = AlphaBetaPruning(directed=False)
                    for i in range(0, self.counter):
                        graphabp.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, best_cost, goal = graphabp.alpha_beta_search(start, goals)
                    if traced_path5:
                        print('Path:', end=' ')
                        graphabp.print_path(traced_path5)
                    Goal_list.clear()
                ...
                if searchType == "BFS":
                    ...
                    graphastar = Graph_astar(directed=True)
                    for i in range(0, self.counter):
                        graphastar.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    ...
                    graphbbfss = Graph_best_first(directed=True)
                    for i in range(0, self.counter):
                        graphbbfss.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    ...
                    graphbd = Graph_bd(directed=True)
                    for i in range(0, self.counter):
                        graphbd.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, goal = graphbd.depth_first_search(start, goals)
                    if traced_path5:
                        print('Path:', end=' ')
                        graphbd.print_path(traced_path5, goal)
                    Goal_list.clear()
                elif searchType == "SimulatedAnnealing":
                    graphsa = SimulatedAnealing(directed=True)
                    for i in range(0, self.counter):
                        graphsa.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    ...
                    graphabp = AlphaBetaPruning(directed=True)
                    for i in range(0, self.counter):
                        graphabp.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
    ...
    def retranslateUi(self, AISearchingTechniquesMainWindow):
        ...
        self.SearchTypecomboBox.setItemText(3, _translate("AISearchingTechniquesMainWindow", "Best First Search"))
        self.SearchTypecomboBox.setItemText(4, _translate("AISearchingTechniquesMainWindow", "Uniform Cost Search"))
        self.SearchTypecomboBox.setItemText(5, _translate("AISearchingTechniquesMainWindow", "Depth limited Search"))
        self.SearchTypecomboBox.setItemText(6, _translate("AISearchingTechniquesMainWindow", "IterativeDeepning"))
        self.SearchTypecomboBox.setItemText(7, _translate("AISearchingTechniquesMainWindow", "Bidirectional"))
        self.SearchTypecomboBox.setItemText(8, _translate("AISearchingTechniquesMainWindow", "SimulatedAnnealing"))
        self.SearchTypecomboBox.setItemText(9, _translate("AISearchingTechniquesMainWindow", "AlphaBetaPrunning"))
